...
import qs from "querystring"; // 截取url的库
...
// 正则匹配，文章与文章下的某一条
Mock.mock(/^\/api\/blog(\?.+)?$/, "get", function(options) {  // options为发送数据请求的信息
  const query = qs.parse(options.url); // 截取url所带参数信息
  const data = {
    "total|2000-3000": 0,
    [`rows|${query.limit || 10}`]: [
      {
        id: "@guid", // 全局唯一
        title: "@ctitle", // 中文标题
        description: "@cparagraph(1, 10)", // 中文段落
        category: {
          "id|1-10": 0,
          name: "分类@id",
        },
        ...
        "thumb|1": ["@image(300x250, @color, #fff, @natural)"], // 固定生成一张图片
