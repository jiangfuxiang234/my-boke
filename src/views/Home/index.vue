<template>
  ...
  v-Loading="loading">
    ...
    <ul class="indicator">
      <li v-for="(item, i) in data" :key="item.id" @click="switchTo(i)" :class="{ active: i === index }">
      </li>
    </ul>
  </div>
</template>
...
export default {
  ...
  created(){ 
    this.$store.dispatch("banner/fetchBanner") // 获取banner信息保存到仓库
  },
  ...
  computed: {
    ...
    ...mapState("banner",["loading","data"]) // 仓库里loading和data的计算属性
  },
  methods: {
    // async fetchData(){ // 注册获取相应数据的fetchData方法
